...
const ModuleActionNameSpace = 'ENUM';
...
export const editItem = (data)=> (dispatch,getState) =>{
    ...
    api.request({
        url:api.ENUM_REST_API + '/' + data.guid,
        method:'PUT',
        data:dataValues
    }).then((res)=>{
        dispatch({
...
export const editItemValue = (item,values) => (dispatch,getState) =>{
    ...
    api.request({
        url:api.ENUM_REST_API + '/' + item.guid + '/' + guid,
        data:dataValues,
        method: 'PUT',
    }).then((res)=>{
        dispatch({
...
export const addItemValue = (item,values)=>(dispatch,getState)=>{
    var {guid,...dataValues} = values;
    api.request({
        url:api.ENUM_REST_API + '/' + item.guid ,
        data:{
            ...dataValues,
            _class:'enumeration.item'   
        },
        method: 'POST',
    }).then((res)=>{
        dispatch({
